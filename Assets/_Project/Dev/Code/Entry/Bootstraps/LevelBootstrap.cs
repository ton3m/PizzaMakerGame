...
using PizzaMaker.Code.Core.Installers;
    ...
    public class LevelBootstrap : IDisposable
    {
        ...
        public IEnumerator Run(DIContainer container)
        {
            ...
            _container.Resolve<GameplayStateNotifier>()
                .NotifyThatEntered(GameplayStates.Loaded);
        ...
        {
            var observer = _container.Resolve<GameEndObserver>();

            observer.Notify(GameResult.None);
            ...
            winObservable.Subscribe(() => observer.Notify(GameResult.Won)).DisposeIn(_disposer);
            loseObservable.Subscribe(() => observer.Notify(GameResult.Lost)).DisposeIn(_disposer);
        }
        ...
        private void RegCharacter()
        {
            _container.RegisterAsSingle(c =>
            {
                ...
                var prefab = _container.Resolve<ResourcesLoader>()
                    .LoadResource<CharacterInstaller>(ResourcesPaths.CharacterPrefab);
                ...
                state
                    .EqualTo(GameplayStates.LevelStarted)
